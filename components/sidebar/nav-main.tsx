...
import { componentRegistry, categoryConfig } from "./component-registry"
...
export function NavMain({ type, label, icon: GroupIcon }: NavMainProps) {
  ...
  const items = Object.entries(componentRegistry).filter(([, item]) => item.type === type)
  ...
  React.useEffect(() => {
    try {
      ...
      const groupStored = localStorage.getItem(`sidebar-${type}-group-open`)
      if (groupStored !== null) {
        setIsGroupOpen(JSON.parse(groupStored))
      }
    } catch {
  ...
  const handleOpenChange = React.useCallback(
    (category: string, open: boolean) => {
      setCollapsedCategories((prev) => {
        ...
        try {
          localStorage.setItem(`sidebar-${type}-collapsed`, JSON.stringify(Array.from(next)))
        } catch {
          // Ignore localStorage errors
        }
        return next
  ...
  const handleGroupOpenChange = React.useCallback(
    (open: boolean) => {
      setIsGroupOpen(open)
      try {
        localStorage.setItem(`sidebar-${type}-group-open`, JSON.stringify(open))
      } catch {
        // Ignore localStorage errors
      }
    },
    ...
    >
      <SidebarGroup>
        ...
        {sortedCategories.map(([category, categoryItems]) => {
          ...
          const categoryLabel = categoryConfig[category]?.label || category.charAt(0).toUpperCase() + category.slice(1)
          const isOpen = !collapsedCategories.has(category)
          
          // Determine if this category contains blocks or components
          const hasBlocks = categoryItems.some(([, item]) => item.type === "registry:block")
          const hasComponents = categoryItems.some(([, item]) => item.type === "registry:ui")
          
          // Use blocks path if category has blocks, otherwise use components path
          const categoryPath = hasBlocks ? `/blocks/${category}` : `/components/${category}`
                ...
                {state === "collapsed" ? (
                  ...
                  >
                    ...
                    <PopoverContent side="right" align="start" className="w-64 p-0">
                      <Command>
                        <CommandInput placeholder={`Search ${categoryLabel.toLowerCase()}...`} className="h-9" />
                        <CommandList>
                          ...
                          <CommandGroup heading={categoryLabel}>
                            ...
                            {categoryItems.map(([key, item]) => (
                              ...
                              >
                                ...
                                <Check
                                  className={cn("ml-auto size-4", pathname === item.href ? "opacity-100" : "opacity-0")}
                                />
                              </CommandItem>
                            ))}
                          </CommandGroup>
                  ...
                  <>
                    <SidebarMenuButton asChild tooltip={categoryLabel} isActive={isCategoryActive}>
