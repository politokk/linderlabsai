import {
  ...
  export const Message = ({ className, from, children, ...props }: MessageProps) => (
    <div
      className={cn(
        "group mb-4 mr-3 flex gap-3",
        from === "user" ? "justify-end is-user" : "justify-start is-assistant",
        className
      )}
      {...props}
    >
      <div className={cn("flex gap-3", from === "user" ? "flex-row-reverse" : "flex-row")}>
        <Avatar className="h-7 w-7 flex-shrink-0">
          {from === "user" ? (
            ...
            <AvatarFallback className="bg-primary-foreground text-primary border border-primary-500">
