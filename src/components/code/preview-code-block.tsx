...
export const CodeBlock = ({
  ...
  const renderCodeBlock = (additionalClassName?: string) => (
    <pre
      className={cn(
        "not-prose flex-1 overflow-x-auto rounded-sm border bg-background p-3 text-sm outline-none",
        "[&>code]:grid",
        className,
        additionalClassName
      )}
  ...
  return (
    <Card className="not-prose mb-6 gap-0 overflow-hidden rounded-sm p-0 shadow-none">
      ...
      <CardContent className="p-0">
        {renderCodeBlock("line-numbers rounded-none border-none")}
      </CardContent>
    </Card>
  );
};
